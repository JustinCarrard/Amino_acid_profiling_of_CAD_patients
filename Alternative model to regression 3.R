#==========================================================================================
# Circulating amino acids in CAD patients (LVEF) vs. healthy controls
...
# Paths
setwd("/Volumes/nuxyde32/Uni-Basel/DSBG/Forschung/COmPLETE Health/Metabolomics/MSc Luisa/CAD/LVEF alternative model") # Main path
...
ggsave(paste(graphics_path, paste("violin_LVEF.png", sep = ""), sep = "/"), p, width = 18*1, height = 14*1, units = "in", dpi = 300)
...
#------------------------------------------------------------------------------------------
my_lms <- lapply(dat.match[,c(116, 118:149)], function(x) lm(x ~ LVEF + Age_std * Sex + Fasting_std + Sampling_time_cat + Total_PA_std + eGFR_std, data = dat.match, weights = weights))
...
df_conf_intervals2 <- subset(df_conf_intervals, Term %in% c("LVEF1", "LVEF2", "LVEF3", "Age_std", "Sex1", "Age_std:Sex1", "Total_PA_std"))
...
for (i in seq_along(names(my_lms))) {
  ...
  res_frame_LVEF1$estimate[which(res_frame_LVEF1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF1", "Estimate"]
  res_frame_LVEF1$std.error[which(res_frame_LVEF1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF1", "Std. Error"]
  res_frame_LVEF1$statistic[which(res_frame_LVEF1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF1", "t value"]
  res_frame_LVEF1$p.value[which(res_frame_LVEF1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF1", "Pr(>|t|)"]
  res_frame_LVEF1$response[which(res_frame_LVEF1$term %in% names(my_lms)[i])] <- "LVEF ≥50%"
  
}  

#extract LVEF2
res_frame_LVEF2 <- data.frame(
  term = names(my_lms)
  , estimate = NA
  , std.error = NA
  , statistic = NA
...
for (i in seq_along(names(my_lms))) {
  ...
  res_frame_LVEF2$estimate[which(res_frame_LVEF2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF2", "Estimate"]
  res_frame_LVEF2$std.error[which(res_frame_LVEF2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF2", "Std. Error"]
  res_frame_LVEF2$statistic[which(res_frame_LVEF2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF2", "t value"]
  res_frame_LVEF2$p.value[which(res_frame_LVEF2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF2", "Pr(>|t|)"]
  res_frame_LVEF2$response[which(res_frame_LVEF2$term %in% names(my_lms)[i])] <- "LVEF =41-49%"

}  

#extract LVEF3
res_frame_LVEF3 <- data.frame(
  term = names(my_lms)
  , estimate = NA
  , std.error = NA
  , statistic = NA
...
for (i in seq_along(names(my_lms))) {
  ...
  res_frame_LVEF3$estimate[which(res_frame_LVEF3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF3", "Estimate"]
  res_frame_LVEF3$std.error[which(res_frame_LVEF3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF3", "Std. Error"]
  res_frame_LVEF3$statistic[which(res_frame_LVEF3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF3", "t value"]
  res_frame_LVEF3$p.value[which(res_frame_LVEF3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["LVEF3", "Pr(>|t|)"]
  res_frame_LVEF3$response[which(res_frame_LVEF3$term %in% names(my_lms)[i])] <- "LVEF ≤40%"
...
write_xlsx(Overall_final,"/Volumes/nuxyde32/Uni-Basel/DSBG/Forschung/COmPLETE Health/Metabolomics/MSc Luisa/CAD/LVEF alternative model/output/text//Regressions_LVEF_alt_mod.xlsx")
...
plot_data$response <- factor(plot_data$response, levels = c("Age", "Male","Interaction age:male", "Daily physical activity", "LVEF ≥50%", "LVEF =41-49%", "LVEF ≤40%"))
...
ggsave(paste(graphics_path, paste("rain_plot_LVEF_alt_mod.png", sep = ""), sep = "/"), p, width = 4*1, height = 9*1, units = "in", dpi = 300)
