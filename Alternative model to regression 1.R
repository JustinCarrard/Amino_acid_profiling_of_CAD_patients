...
# Paths
setwd("/Volumes/nuxyde32/Uni-Basel/DSBG/Forschung/COmPLETE Health/Metabolomics/MSc Luisa/CAD/Healthy vs. sick alternative model") # Main path
...
#------------------------------------------------------------------------------------------
my_lms <- lapply(dat.match[,c(116, 118:149)], function(x) lm(x ~ CAD + Age_std * Sex + Fasting_std + Sampling_time_cat + Total_PA_std + eGFR_std, data = dat.match, weights = weights))
...
write_xlsx(Overall_final,"/Volumes/nuxyde32/Uni-Basel/DSBG/Forschung/COmPLETE Health/Metabolomics/MSc Luisa/CAD/Healthy vs. sick alternative model/output/text//Regressions_CAD_alt_mod.xlsx")
