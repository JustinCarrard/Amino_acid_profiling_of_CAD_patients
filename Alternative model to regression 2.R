#==========================================================================================
# Circulating amino acids in CAD patients (number of affected coronary arteries)
# vs. healthy controls
...
# Paths
setwd("/Volumes/nuxyde32/Uni-Basel/DSBG/Forschung/COmPLETE Health/Metabolomics/MSc Luisa/CAD/No coronary alternative model") # Main path
...
#------------------------------------------------------------------------------------------
# Exclude participants with eGFR < 60
#------------------------------------------------------------------------------------------
...
(variables_to_standardize <- names(dat)[c(2, 21:30, 69, 71, 73:104)])
...
(vars_to_plot <- names(dat)[c(116, 118:149)])
...
ggsave(paste(graphics_path, paste("violin_Number_of_vessels.png", sep = ""), sep = "/"), p, width = 18*1, height = 14*1, units = "in", dpi = 300)
...
#------------------------------------------------------------------------------------------
my_lms <- lapply(dat.match[,c(116, 118:149)], function(x) lm(x ~ Number_of_vessels + Age_std * Sex + Fasting_std + Sampling_time_cat + Total_PA_std, data = dat.match, weights = weights))
...
df_conf_intervals2 <- subset(df_conf_intervals, Term %in% c("Number_of_vessels1", "Number_of_vessels2", "Number_of_vessels3", "Age_std", "Sex1", "Age_std:Sex1", "Total_PA_std"))
...
df_conf_intervals4 <- df_conf_intervals3 %>%
  mutate(Term = case_when(
    Term == "Number_of_vessels1" ~ "Single-vessel coronary artery disease",
    Term == "Number_of_vessels2" ~ "Double-vessel coronary artery disease",
    Term == "Number_of_vessels3" ~ "Triple-vessel coronary artery disease",
    Term == "Sex1" ~ "Male",
    Term == "Age_std" ~ "Age",
...
#extract Number_of_vessels1
res_frame_Number_of_vessels1 <- data.frame(
  term = names(my_lms)
  , estimate = NA
  , std.error = NA
  , statistic = NA
...
for (i in seq_along(names(my_lms))) {
  ...
  res_frame_Number_of_vessels1$estimate[which(res_frame_Number_of_vessels1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels1", "Estimate"] 
  res_frame_Number_of_vessels1$std.error[which(res_frame_Number_of_vessels1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels1", "Std. Error"]
  res_frame_Number_of_vessels1$statistic[which(res_frame_Number_of_vessels1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels1", "t value"]
  res_frame_Number_of_vessels1$p.value[which(res_frame_Number_of_vessels1$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels1", "Pr(>|t|)"]
  res_frame_Number_of_vessels1$response[which(res_frame_Number_of_vessels1$term %in% names(my_lms)[i])] <- "Single-vessel coronary artery disease"

}  

#extract Number_of_vessels2
res_frame_Number_of_vessels2 <- data.frame(
  term = names(my_lms)
  , estimate = NA
  , std.error = NA
  , statistic = NA
...
for (i in seq_along(names(my_lms))) {
  ...
  res_frame_Number_of_vessels2$estimate[which(res_frame_Number_of_vessels2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels2", "Estimate"] 
  res_frame_Number_of_vessels2$std.error[which(res_frame_Number_of_vessels2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels2", "Std. Error"]
  res_frame_Number_of_vessels2$statistic[which(res_frame_Number_of_vessels2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels2", "t value"]
  res_frame_Number_of_vessels2$p.value[which(res_frame_Number_of_vessels2$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels2", "Pr(>|t|)"]
  res_frame_Number_of_vessels2$response[which(res_frame_Number_of_vessels2$term %in% names(my_lms)[i])] <- "Double-vessel coronary artery disease"
  
}  

#extract Number_of_vessels3
res_frame_Number_of_vessels3 <- data.frame(
  term = names(my_lms)
  , estimate = NA
  , std.error = NA
  , statistic = NA
...
for (i in seq_along(names(my_lms))) {
  ...
  res_frame_Number_of_vessels3$estimate[which(res_frame_Number_of_vessels3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels3", "Estimate"] 
  res_frame_Number_of_vessels3$std.error[which(res_frame_Number_of_vessels3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels3", "Std. Error"]
  res_frame_Number_of_vessels3$statistic[which(res_frame_Number_of_vessels3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels3", "t value"]
  res_frame_Number_of_vessels3$p.value[which(res_frame_Number_of_vessels3$term %in% names(my_lms)[i])] <- coef_results[[names(my_lms)[i]]]["Number_of_vessels3", "Pr(>|t|)"]
  res_frame_Number_of_vessels3$response[which(res_frame_Number_of_vessels3$term %in% names(my_lms)[i])] <- "Triple-vessel coronary artery disease"
...
#Combine_data_frames
Overall <- rbind(
  res_frame_Number_of_vessels1
  , res_frame_Number_of_vessels2
  , res_frame_Number_of_vessels3
  , res_frame_Age
  , res_frame_Sex
  , res_frame_Age_Sex
  , res_frame_Total_PA
...
write_xlsx(Overall_final,"/Volumes/nuxyde32/Uni-Basel/DSBG/Forschung/COmPLETE Health/Metabolomics/MSc Luisa/CAD/No coronary alternative model/output/text/Regressions_no_coronary_alt_mod.xlsx")
...
plot_data$response <- factor(plot_data$response, levels = c("Age", "Male", "Interaction age:male", "Daily physical activity", "Single-vessel coronary artery disease", "Double-vessel coronary artery disease", "Triple-vessel coronary artery disease"))
...
ggsave(paste(graphics_path, paste("rain_plot_No_coronary_alt_mod.png", sep = ""), sep = "/"), p, width = 4*1, height = 9*1, units = "in", dpi = 300)
