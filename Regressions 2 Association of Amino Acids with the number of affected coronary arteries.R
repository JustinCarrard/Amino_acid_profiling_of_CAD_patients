...
#------------------------------------------------------------------------------------------
my_lms <- lapply(dat.match[,c(114:146)], function(x) lm(x ~ Number_of_vessels + Age_std * Sex + Fasting_std + Sampling_time_cat + Total_PA_std, data = dat.match, weights = weights))
...
write_xlsx(Overall_final, text_path, "Regressions_no_coronary.xlsx")
...
ggsave(paste(graphics_path, paste("rain_plot_No_coronary.png", sep = ""), sep = "/"), p, width = 4*1, height = 9*1, units = "in", dpi = 300)
